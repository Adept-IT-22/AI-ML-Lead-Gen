...
async def no_rate_limit_people_search(
    ...
    # per_page=3 caps Apollo credit cost to 3 per company.
    # Apollo ranks results internally by relevance, so the 3 returned
    # are already its best matches given the seniority filter.
    payload = {
        "person_seniorities": [
    ...
    try:
        ...
        logger.info("People search complete — %d people returned for %r", len(data.get("people", [])), org_domains)
        return data
