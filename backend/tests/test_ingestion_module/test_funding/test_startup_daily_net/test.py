...
from ingestion_module.funding.startup_daily_net import fetch
...
@pytest.mark.asyncio
async def test_fetch_startup_daily_net_data_happy_path(monkeypatch, caplog):
    """
    Tests the two-level sitemap filtering for startupdaily.net.
    ...
    <sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
        ...
        <sitemap>
            <loc>https://startupdaily.net/post-sitemap10.xml</loc>
            <lastmod>{recent_date_str}</lastmod>
        </sitemap>
        <!-- Case 2: Should be IGNORED (same recent date, lower number) -->
        <sitemap>
            <loc>https://startupdaily.net/post-sitemap9.xml</loc>
            <lastmod>{recent_date_str}</lastmod>
        </sitemap>
        <!-- Case 3: Should be IGNORED (older date) -->
        <sitemap>
            <loc>https://startupdaily.net/post-sitemap8.xml</loc>
            <lastmod>{slightly_older_date_str}</lastmod>
        </sitemap>
        <!-- Case 4: Should be IGNORED (not a 'post-sitemap') -->
        <sitemap>
            <loc>https://startupdaily.net/page-sitemap.xml</loc>
            <lastmod>{recent_date_str}</lastmod>
        </sitemap>
        <!-- Case 5: Should be IGNORED (same recent date, lower number than 10) -->
        <sitemap>
            <loc>https://startupdaily.net/post-sitemap.xml</loc>
            <lastmod>{recent_date_str}</lastmod>
        </sitemap>
    </sitemapindex>
    """
    ...
    <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
        ...
        <url>
            <loc>https://startupdaily.net/2024/05/23/indian-american-led-ai-startup-secures-funding/</loc>
            <lastmod>{recent_date_str}</lastmod>
        </url>
        <!-- Article 2: Should be IGNORED (old date) -->
        <url>
            <loc>https://startupdaily.net/2023/01/01/old-story-about-ai-funding/</loc>
            <lastmod>{old_date_str}</lastmod>
        </url>
        <!-- Article 3: Should be IGNORED (no funding keyword) -->
        <url>
            <loc>https://startupdaily.net/2024/05/22/new-ai-technology-unveiled/</loc>
            <lastmod>{recent_date_str}</lastmod>
        </url>
    </urlset>
    """
    ...
    async def mock_fetch(client, url):
        if url == "https://www.startupdaily.net/sitemap_index.xml":
            return MockResponse(sitemap_index_xml)
        elif url == "https://startupdaily.net/post-sitemap10.xml":
    ...
    results = await fetch.fetch_startup_daily_net_data()

    # Assertions
    assert "Processing latest sitemap: https://startupdaily.net/post-sitemap10.xml" in caplog.text
    assert len(results["urls"]) == 1
    assert results["urls"][0] == "https://startupdaily.net/2024/05/23/indian-american-led-ai-startup-secures-funding/"
    assert "https://startupdaily.net/2024/05/22/new-ai-technology-unveiled/" not in results["urls"]
    assert "Paragraph from https://startupdaily.net/2024/05/23/indian-american-led-ai-startup-secures-funding/" in results["paragraphs"]

@pytest.mark.asyncio
async def test_extract_paragraphs_from_startup_daily_net_html(monkeypatch):
    """
    Tests that paragraphs are correctly extracted from startupdaily.net's specific HTML structure.
    """
    html_content = """
    <html><body>
    ...
    """
    test_url = "https://startupdaily.net/fake-article"
...
async def test_main_integration_with_ai_extraction(monkeypatch):
    ...
    """
    fake_fetched_data = {
        "urls": ["https://startupdaily.net/some-ai-funding-story/"],
        "paragraphs": ["A paragraph about AI and funding."]
    }
    monkeypatch.setattr(fetch, "fetch_startup_daily_net_data", AsyncMock(return_value=fake_fetched_data))
    ...
    assert result["source"] == ["startupdailynet"]
    assert result["company_name"] == ["Test AI Co"]
    assert result["link"] == ["https://startupdaily.net/some-ai-funding-story/"]
