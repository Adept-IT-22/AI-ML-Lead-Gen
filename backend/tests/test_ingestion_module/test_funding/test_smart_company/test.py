...
from ingestion_module.funding.smart_company import fetch
...
@pytest.mark.asyncio
async def test_fetch_smart_company_data_happy_path(monkeypatch, caplog):
    """
    Tests the two-level sitemap filtering for smartcompany.com.au.
    ...
    <sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
        ...
        <sitemap>
            <loc>https://www.smartcompany.com.au/post-sitemap55.xml</loc>
            <lastmod>{recent_date_str}</lastmod>
        </sitemap>
        <!-- Case 2: Should be IGNORED (same recent date, lower number) -->
        <sitemap>
            <loc>https://www.smartcompany.com.au/post-sitemap54.xml</loc>
            <lastmod>{recent_date_str}</lastmod>
        </sitemap>
        <!-- Case 3: Should be IGNORED (older date) -->
        <sitemap>
            <loc>https://www.smartcompany.com.au/post-sitemap53.xml</loc>
            <lastmod>{slightly_older_date_str}</lastmod>
        </sitemap>
        <!-- Case 4: Should be IGNORED (not a 'post-sitemap') -->
        <sitemap>
            <loc>https://www.smartcompany.com.au/page-sitemap.xml</loc>
            <lastmod>{recent_date_str}</lastmod>
        </sitemap>
    </sitemapindex>
    """
    ...
    <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
        ...
        <url>
            <loc>https://www.smartcompany.com.au/startups/ai-startup-raises-funding-round/</loc>
            <lastmod>{recent_date_str}</lastmod>
        </url>
        <!-- Article 2: Should be IGNORED (old date) -->
        <url>
            <loc>https://www.smartcompany.com.au/startups/old-story-about-ai-funding/</loc>
            <lastmod>{old_date_str}</lastmod>
        </url>
        <!-- Article 3: Should be IGNORED (no funding keyword) -->
        <url>
            <loc>https://www.smartcompany.com.au/startups/new-ai-technology-unveiled/</loc>
            <lastmod>{recent_date_str}</lastmod>
        </url>
    </urlset>
    """
    ...
    async def mock_fetch(client, url):
        if url == "https://www.smartcompany.com.au/sitemap_index.xml":
            return MockResponse(sitemap_index_xml)
        elif url == "https://www.smartcompany.com.au/post-sitemap55.xml":
    ...
    results = await fetch.fetch_smart_company_data()

    # Assertions
    assert "Processing latest sitemap: https://www.smartcompany.com.au/post-sitemap55.xml" in caplog.text
    assert len(results["urls"]) == 1
    assert results["urls"][0] == "https://www.smartcompany.com.au/startups/ai-startup-raises-funding-round/"
    assert "https://www.smartcompany.com.au/startups/new-ai-technology-unveiled/" not in results["urls"]
    assert "Paragraph from https://www.smartcompany.com.au/startups/ai-startup-raises-funding-round/" in results["paragraphs"]

@pytest.mark.asyncio
async def test_extract_paragraphs_from_smart_company_html(monkeypatch):
    """
    Tests that paragraphs are correctly extracted from smartcompany.com.au's specific HTML structure.
    """
    html_content = """
    <html><body>
    ...
    """
    test_url = "https://www.smartcompany.com.au/fake-article"
...
async def test_main_integration_with_ai_extraction(monkeypatch):
    ...
    """
    fake_fetched_data = {
        "urls": ["https://www.smartcompany.com.au/some-ai-funding-story/"],
        "paragraphs": ["A paragraph about AI and funding."]
    }
    monkeypatch.setattr(fetch, "fetch_smart_company_data", AsyncMock(return_value=fake_fetched_data))
    ...
    assert result["source"] == ["smartcompany"]
    assert result["company_name"] == ["Test AI Co"]
    assert result["link"] == ["https://www.smartcompany.com.au/some-ai-funding-story/"]
