...
#============URLs==============
URL = ["https://www.startupdaily.net/sitemap_index.xml"]
...
async def fetch_startup_daily_net_data() -> Dict[str, List[str]]:
    logger.info("Fetching data from startupdailynet...")
    ...
    try:
        ...
        logger.info(f"Done fetching data from startupdailynet. Found {len(results['urls'])} articles.")
        return results

    except Exception as e:
        logger.exception(f"Error fetching/parsing startupdailynet sitemaps: {e}")
        return {"urls": [], "paragraphs": []}
    finally:
        client.close()
...
async def extract_paragraphs(client: cloudscraper.CloudScraper, url: str)->tuple[str, List[str]]:
    ...
    try:
        ...
        paragraph_nodes = root.xpath("//section[contains(@class, 'entry-content')]//p")
...
async def main() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Failed to extract AI content from startupdailynet data: {e}")
        result = {}
    ...
    llm_results["source"].append("startupdailynet")
    llm_results["link"] = links_and_paragraphs.get("urls", [])

    duration = time.perf_counter() - start_time
    logger.info(f"startupdailynet took {duration:.2f} seconds")
