...
#============URLs==============
URL = ["https://inc42.com/news-sitemap.xml"]
...
async def fetch_inc42_data() -> Dict[str, List[str]]:
    logger.info("Fetching data from Inc42...")
    ...
    try:
        ...
        for url_node in sitemap_root.findall(".//sitemap_ns:url", namespaces):
            ...
            news_el = url_node.find("news:news", namespaces)

            if loc_el is None or news_el is None:
                continue

            date_el = news_el.find("news:publication_date", namespaces)
            title_el = news_el.find("news:title", namespaces)

            if loc_el.text is None or date_el is None or date_el.text is None or title_el is None or title_el.text is None:
                continue
            ...
            if pub_date.year == now.year and pub_date.month in allowed_months:
                if FUNDING_KEYWORDS_REGEX.search(title) and AI_KEYWORDS_REGEX.search(title):
        ...
        logger.info("Done fetching data from Inc42")
        return results

    except Exception as e:
        logger.exception(f"An error occurred during the Inc42 fetching process: {str(e)}")
...
async def extract_paragraphs(client: cloudscraper.CloudScraper, url: str)->tuple[str, List[str]]:
    ...
    try:
        ...
        # This is a common XPath for WordPress article content, which inc42 uses
...
async def main() -> Dict[str, Any]:
    ...
    async with limiter:
        links_and_paragraphs = await fetch_inc42_data()
        ...
        if links_and_paragraphs and (links_and_paragraphs.get("urls") and links_and_paragraphs.get("paragraphs")):
            try:
                ...
                logger.error(f"Failed to extract AI content from Inc42's data: {str(e)}")
                result = {}
        else:
        ...
        if result:
            ...
            llm_results["source"].append("inc42")
            urls = links_and_paragraphs.get("urls")
            llm_results["link"] = urls

        else:
            logger.warning("AI extraction for Inc42 returned no data. No logging will happen")

        duration = time.perf_counter() - start_time
        logger.info(f"Inc42 took {duration:.2f} seconds")
