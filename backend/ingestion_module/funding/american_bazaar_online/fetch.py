...
#============URLs==============
URL = ["https://americanbazaaronline.com/sitemap_index.xml"]
...
async def fetch_american_bazaar_data() -> Dict[str, List[str]]:
    logger.info("Fetching data from americanbazaaronline...")
    ...
    try:
        ...
        for sitemap_node in index_root.findall(".//sitemap_ns:sitemap", namespaces):
            ...
            if loc_el is not None and loc_el.text and "post-sitemap" in loc_el.text and ".xml" in loc_el.text and lastmod_el is not None and lastmod_el.text:
                ...
                try:
                    ...
                    # Handle case where number is missing (e.g., post-sitemap.xml) -> treat as 0
                    num_str = match.group(1) if match else ''
                    current_sitemap_number = int(num_str) if num_str else 0
        ...
        # Check articles from the last 60 days
        time_threshold = now - datetime.timedelta(days=60)
        ...
        logger.info(f"Done fetching data from americanbazaaronline. Found {len(results['urls'])} articles.")
        return results

    except Exception as e:
        logger.exception(f"Error fetching/parsing americanbazaaronline sitemaps: {e}")
        return {"urls": [], "paragraphs": []}
    finally:
        client.close()
...
async def main() -> Dict[str, Any]:
    ...
    links_and_paragraphs = await fetch_american_bazaar_data()

    if not (links_and_paragraphs and links_and_paragraphs.get("urls")):
        logger.info("No articles found from americanbazaaronline to process for AI extraction.")
        return copy.deepcopy(funding_data_dict)

    try:
        result = await finalize_ai_extraction(links_and_paragraphs=links_and_paragraphs)
    except Exception as e:
        logger.error(f"Failed to extract AI content from americanbazaaronline data: {e}")
        result = {}

    if not result:
        logger.warning("AI extraction for americanbazaaronline returned no data. No logging will happen")
        return copy.deepcopy(funding_data_dict)
    ...
    llm_results["source"].append("americanbazaaronline")
    llm_results["link"] = links_and_paragraphs.get("urls", [])

    duration = time.perf_counter() - start_time
    logger.info(f"americanbazaaronline took {duration:.2f} seconds")
