...
#============URLs==============
URL = ["https://www.siliconrepublic.com/sitemap_index.xml"]
...
async def fetch_with_cloudscraper(client: cloudscraper.CloudScraper, url: str):
    # wrap cloudscraper with an async rate limiter
    async with limiter:
...
async def fetch_silicon_republic_data() -> Dict[str, List[str]]:
    logger.info("Fetching data from siliconrepublic.com...")
    ...
    try:
        ...
        # Find all recent 'post-sitemap' URLs
        for sitemap_node in index_root.findall(".//sitemap_ns:sitemap", namespaces):
            ...
            if loc_el is not None and loc_el.text and "post-sitemap" in loc_el.text and ".xml" in loc_el.text and lastmod_el is not None and lastmod_el.text:
                try:
                    sitemap_mod_date = datetime.datetime.fromisoformat(lastmod_el.text.replace('Z', '+00:00'))
                    
                    # Filter by date
                    if sitemap_mod_date.year == now.year and sitemap_mod_date.month in allowed_months:
                        child_sitemap_urls_to_process.append(loc_el.text)
                except ValueError:
                    logger.warning(f"Could not parse date {lastmod_el.text} for sitemap {loc_el.text}")
                    continue

        if not child_sitemap_urls_to_process:
            logger.error("Could not find any recent post-sitemaps. Aborting.")
            return {"urls": [], "paragraphs": []}

        logger.info(f"Found {len(child_sitemap_urls_to_process)} recent post-sitemaps to process.")

        # Process each identified child sitemap sequentially.
        for sitemap_url in child_sitemap_urls_to_process:
            try:
                sub_sitemap_response = await fetch_with_cloudscraper(client, sitemap_url)
        ...
        logger.info("Done fetching data from siliconrepublic.com")
        return results
    except Exception as e:
        logger.exception(f"Error fetching/parsing siliconrepublic.com sitemaps: {e}")
...
async def extract_paragraphs(client: cloudscraper.CloudScraper, url: str)->tuple[str, List[str]]:
    ...
    try:
        ...
        paragraph_nodes = root.xpath("//div[contains(@itemprop, 'articleBody')]//p")
...
async def main() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Failed to extract AI content from siliconrepublic.com's data: {e}")
        result = {}
    ...
    logger.info(f"siliconrepublic took {duration:.2f} seconds")
