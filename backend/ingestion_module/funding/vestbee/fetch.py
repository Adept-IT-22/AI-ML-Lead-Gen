...
#============URLs==============
URL = ["https://www.vestbee.com/__sitemap__/posts.xml"]
...
async def fetch_vestbee_data() -> Dict[str, List[str]]:
    logger.info("Fetching data from vestbee...")
    ...
    try:
        ...
        logger.info(f"Scraping sitemap: {URL[0]}") # Access the first URL in the list
        ...
        logger.info("Done fetching data from vestbee")
        return results

    except Exception as e:
        logger.exception(f"An error occurred during the vestbee fetching process: {str(e)}")
...
async def extract_paragraphs(client: cloudscraper.CloudScraper, url: str)->tuple[str, List[str]]:
    ...
    try:
        ...
        paragraph_nodes = root.xpath("//div[contains(@class, 'article-content')]//p")
...
async def main() -> Dict[str, Any]:
    ...
    async with limiter:
        links_and_paragraphs = await fetch_vestbee_data()
        ...
        if links_and_paragraphs and (links_and_paragraphs.get("urls") and links_and_paragraphs.get("paragraphs")):
            try:
                ...
                logger.error(f"Failed to extract AI content from vestbee's data: {str(e)}")
                result = {}
        else:
        ...
        if result:
            ...
            logger.warning("AI extraction for vestbee returned no data. No logging will happen")

        duration = time.perf_counter() - start_time
        logger.info(f"vestbee took {duration:.2f} seconds")
