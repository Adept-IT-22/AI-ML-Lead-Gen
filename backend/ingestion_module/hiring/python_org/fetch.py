...
async def fetch_rss_content() -> str:
    """Fetch jobs from Python.org RSS feed."""
    async with httpx.AsyncClient() as client:
        try:
            ...
            logger.error(f"Error fetching Python.org RSS: {str(e)}")
            return ""

def parse_rss(xml_content: str) -> List[Dict[str, Any]]:
    """Parse Python.org RSS content."""
    jobs = []
    try:
        ...
        logger.error(f"Failed to parse Python.org RSS XML: {str(e)}")
...
async def main() -> Optional[Dict[str, Any]]:
    ...
    logger.info(f"Fetched {len(raw_jobs)} jobs from Python.org")
    ...
    # CHECK IF URL EXISTS IN NORMALIZED_MASTER
    ...
    ids_urls_titles = {
        ...
        "titles": [f"{job['title']}. Content: {job['description'][:1000]}" for job in targets]
    }

    extracted_data = {}
    try:
        logger.info(f"Feeding {len(targets)} Python.org jobs to AI extraction...")
        extracted_data = await finalize_ai_extraction(ids_urls_titles)
    except Exception as e:
        logger.error(f"AI extraction failed for Python.org: {e}")

    # Final result structure
    llm_results = copy.deepcopy(fetched_hiring_data)
    llm_results["source"] = "Python.org"
