...
import logging
import json
from ingestion_module.funding.sifted_eu.fetch import main as sifted_main
...
async def test_sifted():
    logger.info("=" * 50)
    logger.info("TESTING SIFTED.EU MODULE")
    logger.info("=" * 50)
    
    try:
        result = await sifted_main()
        ...
        if result:
            ...
            with open('sifted_test_result.json', 'w', encoding='utf-8') as f:
                json.dump(result, f, indent=2, ensure_ascii=False)
            
            logger.info("\n✅ Full results saved to 'sifted_test_result.json'")
            
        else:
            logger.warning("❌ No results returned from sifted.eu module")
