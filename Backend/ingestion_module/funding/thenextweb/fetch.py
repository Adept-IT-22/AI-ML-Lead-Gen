...
#============URLs==============
URL = ["https://thenextweb.com/sitemap-articles-index.xml"]
...
async def fetch_thenextweb_data() -> Dict[str, List[str]]:
    logger.info("Fetching data from thenextweb...")
        ...
        try:
            ...
            for response in top_level_responses:
                try:
                    ...
                    # Find recent CHILD sitemaps from the index xml file
                    for sitemap_node in root.findall(".//ns:sitemap", namespaces):
                        loc_element = sitemap_node.find("ns:loc", namespaces)
                        lastmod_element = sitemap_node.find("ns:lastmod", namespaces)
                        if loc_element is not None and lastmod_element is not None and lastmod_element.text:
                            try:
                                pub_date = datetime.datetime.fromisoformat(lastmod_element.text.replace('Z', '+00:00')).astimezone(datetime.timezone.utc)
                                if pub_date.year == now.year and pub_date.month in allowed_months:
                                    child_sitemap_urls.append(loc_element.text)
            ...
            logger.info(f"Found {len(child_sitemap_urls)} recent child sitemaps to process.")

            # Fetch recent child sitemaps
            child_sitemap_tasks = [fetch_with_retries(client, url) for url in child_sitemap_urls]
            child_sitemap_responses = await asyncio.gather(*child_sitemap_tasks)

            # Process article links from child sitemaps
                ...
                try:
                    ...
                    for url_node in root.findall(".//ns:url", namespaces):
                        ...
                        if loc_el is not None and loc_el.text and lastmod_el is not None and lastmod_el.text:
                            try:
                                # First filter = article's own date
                                pub_date = datetime.datetime.fromisoformat(lastmod_el.text.replace('Z', '+00:00')).astimezone(datetime.timezone.utc)
                                if pub_date.year == now.year and pub_date.month in allowed_months:
                                ...
                                # Ignore if date parsing fails for an individual article ***********EXPLAIN***************
                                continue
                except XMLSyntaxError as e:
                    logger.error(f"Failed to parse child sitemap XML from {response.url}: {str(e)}")
                    continue
            ...
            logger.info("Done fetching data from thenextweb")
            return results

        except Exception as e:
            logger.exception(f"Error fetching/parsing thenextweb sitemaps: {str(e)}")
...
async def extract_paragraphs(client: httpx.AsyncClient, url: str)->tuple[str, List[str]]:
    ...
    try:
        ...
        paragraph_nodes = root.xpath("//div[contains(@class, 'c-article__main')]//p")
...
async def main():
    ...
    links_and_paragraphs = await fetch_thenextweb_data()
    ...
    if links_and_paragraphs and (links_and_paragraphs.get("urls") and links_and_paragraphs.get("paragraphs")):
        try:
            ...
            logger.error(f"Failed to extract AI content from next web's data: {str(e)}")
            result = {}
    else:
    ...
    if result:
        ...
        llm_results["source"].append("thenextweb.com")
        urls = links_and_paragraphs.get("urls")
        llm_results["link"] = urls

    else:
        logger.warning("AI extraction for thenextweb returned no data. No logging will happen")
